...
export async function SubSectionCard({ data, sectionCount }) {
  const { fields } = data
                ...
                <div className="mt-6 grid gap-6 md:grid-cols-2">
                  {
                    subCollection?.map((item, i) => (
                      <MotionVariant variants={fadeInFromBottom} key={i} className="border bg-white p-4 md:p-8">
                        <p className="font-heading text-lg font-semibold uppercase tracking-wide text-secondary-900">{item.name}</p>
                        <p className="mt-1 text-gray-600 md:text-lg">{_.get(item, 'fields.body')}</p>
                      </MotionVariant>
                    ))
                  }
                </div>
